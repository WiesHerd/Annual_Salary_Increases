...
export function Layout({ children, currentView, onNavigate, sidebarHidden = false, contentNoPadding = false }: LayoutProps) {
  ...
  const [selectedCycleId, setSelectedCycleId] = useSelectedCycle();
          ...
          <div className="shrink-0 border-b border-slate-100 px-4 py-3">
            <label className="flex flex-col gap-1.5">
              ...
              <select
                ...
                className="min-w-0 rounded-md border border-slate-200 bg-slate-50 px-2.5 py-1.5 text-sm text-slate-800 focus:border-indigo-500 focus:outline-none focus:ring-1 focus:ring-indigo-500/30"
              ...
              {section.items.map((item) => {
                ...
                return (
                  <button
                    ...
                    className={`w-full flex items-center gap-3 px-4 py-2.5 text-sm text-left transition-colors border-l-[3px] ${
                      isActive
                        ? 'sidebar-nav-active'
