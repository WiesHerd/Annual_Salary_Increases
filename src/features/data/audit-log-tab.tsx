/**
 * Data browser tab: view and export activity audit log and local error log.
 */
...
import { loadErrorLogEntries, type ErrorLogEntry } from '../../lib/error-log';
...
function actorLabel(entry: AuditEntry): string {
  return entry.userLabel ?? entry.userId ?? '—';
}
...
export function AuditLogTab() {
  ...
  useEffect(() => {
    ...
    return () => window.removeEventListener(AUDIT_UPDATED_EVENT, onUpdate);
  }, [reload]);
  ...
  const errorEntries = useMemo(() => {
    void refreshKey;
    return [...loadErrorLogEntries()].reverse();
  }, [refreshKey]);
  ...
  if (allEntries.length === 0 && errorEntries.length === 0) {
    return (
      <EmptyStatePanel
        title="Audit log"
        message="No activity recorded yet. Sign-in, imports, exports, and provider edits are logged here."
        compact
      />
    );
  }
    ...
    <div className="space-y-4">
      <div className="app-card overflow-hidden flex flex-col min-w-0">
        <div className="shrink-0 border-b border-slate-200 px-5 py-4 flex flex-wrap items-center justify-between gap-3">
          <div>
            <h2 className="text-xl font-semibold text-slate-800">Activity log ({allEntries.length})</h2>
            <p className="text-sm text-slate-500 mt-0.5">
              Sign-in, imports, exports, and data changes — stored on this device. Optional cloud copy when Supabase is
              configured.
            </p>
          </div>
          <button
        ...
        <div className="shrink-0 border-b border-slate-100 px-5 py-3 flex flex-wrap items-center gap-3">
          <input
            ...
            placeholder="Search by ID, field, user, or value…"
        ...
        {allEntries.length === 0 ? (
          <div className="px-5 py-8 text-sm text-slate-500">No activity entries yet.</div>
        ) : (
          <>
                ...
                <tbody>
                  {paginated.map((e: AuditEntry) => (
                    <tr key={e.id}>
                      ...
                      <td className="text-sm text-slate-600 max-w-[140px] truncate" title={actorLabel(e)}>
                        {actorLabel(e)}
                      </td>
      ...
      {errorEntries.length > 0 && (
        <div className="app-card overflow-hidden">
          <div className="border-b border-slate-200 px-5 py-4">
            <h3 className="text-lg font-semibold text-slate-800">Error log ({errorEntries.length})</h3>
            <p className="text-sm text-slate-500 mt-0.5">
              App errors captured on this device — no Sentry or third-party service required.
            </p>
          </div>
          <div className="overflow-x-auto">
            <table className="app-data-table min-w-full">
              ...
              <tbody>
                {errorEntries.slice(0, 25).map((e: ErrorLogEntry) => (
                  <tr key={e.id}>
                    <td className="whitespace-nowrap text-sm text-slate-600">
                      {new Date(e.timestamp).toLocaleString()}
                    </td>
                    <td className="text-sm text-slate-600">{e.userLabel}</td>
                    <td className="text-sm text-slate-500">{e.source}</td>
                    <td className="text-sm text-slate-800">{e.message}</td>
                  </tr>
                ))}
              </tbody>
            </table>
